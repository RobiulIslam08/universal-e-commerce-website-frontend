...
export default function AdminReviewsPage() {
  ...
  const handleDelete = (reviewId: string) => {
    ...
    startTransition(async () => {
      try {
        ...
        setActionMessage({ type: "error", text: "Failed" });
      }
    });
  };
  ...
  const handleToggleApproval = (reviewId: string, currentStatus: boolean) => {
    startTransition(async () => {
      try {
        ...
        setActionMessage({
          type: "success",
          text: currentStatus ? "Disapproved" : "Approved",
        });
        fetchReviews();
  ...
  const getProductInfo = (review: IReview) => {
    if (typeof review.productId === "object" && review.productId !== null) {
      return {
        title: review.productId.title || "Unknown",
        image: review.productId.images?.[0] || null,
      };
    }
  ...
  return (
    <div className="p-3 sm:p-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
      {/* Header & Stats (আগের মতোই থাকবে) */}
      <div className="mb-4 sm:mb-8">
        <h1 className="text-xl sm:text-3xl font-bold text-slate-800 dark:text-slate-200">📝 Reviews</h1>
      </div>

      {/* Filter Tabs */}
      <div className="flex gap-1 overflow-x-auto pb-2 mb-4">
        {(["all", "approved", "pending"] as const).map((tab) => (
          <button key={tab} onClick={() => setFilter(tab)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${filter === tab ? "bg-rose-600 text-white" : "bg-white dark:bg-slate-800 border"}`}>
            {tab}
          </button>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        <>
          {/* 🖥️ Desktop Table View (Visible on Medium Screens & Up) */}
          <div className="hidden md:block bg-white dark:bg-slate-800 rounded-xl border border-slate-200 overflow-hidden">
                ...
                {filteredReviews.map((review) => {
                  ...
                  return (
                    <tr key={review._id} className="hover:bg-slate-50/50">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <Image src={productInfo.image || "/placeholder.png"} alt="" width={40} height={40} className="rounded object-cover" />
                          <span className="font-medium truncate max-w-[120px]">{productInfo.title}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${review.isApproved ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                          {review.isApproved ? "Approved" : "Pending"}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex justify-center gap-2">
                          <button onClick={() => handleToggleApproval(review._id, review.isApproved)} className="p-2 hover:bg-slate-100 rounded">
                             {review.isApproved ? "🚫" : "✅"}
                          </button>
                          <button onClick={() => handleDelete(review._id)} className="p-2 hover:bg-red-50 text-red-500 rounded">
                             🗑️
                          </button>
                        </div>
                      </td>
          ...
          <div className="grid grid-cols-1 gap-3 md:hidden">
            {filteredReviews.map((review) => {
              const productInfo = getProductInfo(review);
              return (
                <div key={review._id} className="p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200">
                  <div className="flex gap-3 mb-2">
                    <Image src={productInfo.image || "/placeholder.png"} alt="" width={40} height={40} className="rounded object-cover h-10 w-10" />
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] font-bold text-rose-600 uppercase">{productInfo.title}</p>
                      <p className="text-xs font-semibold">{review.reviewerName}</p>
                      <StarRating rating={review.rating} />
                    </div>
                  </div>
                  <p className="text-[11px] text-slate-500 bg-slate-50 p-2 rounded mb-2 line-clamp-2">{review.comment}</p>
                  <div className="flex justify-between items-center border-t pt-2">
                    <span className="text-[10px] font-bold uppercase text-slate-400">{new Date(review.createdAt).toLocaleDateString()}</span>
                    <div className="flex gap-2">
                       <button onClick={() => handleToggleApproval(review._id, review.isApproved)} className="text-[10px] font-bold text-blue-600 uppercase">
                        {review.isApproved ? "Reject" : "Approve"}
                       </button>
                       <button onClick={() => handleDelete(review._id)} className="text-[10px] font-bold text-red-600 uppercase">Delete</button>
                    </div>
                  </div>
                </div>
              );
