    ...
    >
      <CardContent className="p-4 sm:p-6">
        <div className="flex gap-4"> 
          ...
          <div className="pt-2 shrink-0">
            <Checkbox 
              id={`item-${item._id}`}
              checked={isSelected}
              onCheckedChange={(checked) => onToggleSelection(item._id, checked as boolean)}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start gap-4 mb-3">
                ...
                <div className="hidden sm:flex gap-1 shrink-0">
                  <Button variant="ghost" size="icon" onClick={() => moveToWishlist(item._id)} title="Move to Wishlist">
                    <Heart className="w-4 h-4 text-slate-400 hover:text-rose-500 hover:fill-rose-500 transition-all" />
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => removeItem(item._id)} title="Remove Item">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="px-3 font-semibold text-lg">{item.orderQuantity}</span>
                  <Button 
                    variant="outline" 
                    size="icon" 
        ...
        <div className="flex justify-end gap-3 mt-4 sm:hidden border-t pt-4">
          <Button variant="ghost" className="text-slate-500 hover:text-rose-500" onClick={() => moveToWishlist(item._id)}><Heart className="w-4 h-4 mr-2" /> Wishlist</Button>
          <Button variant="ghost" className="text-slate-500 hover:text-red-500" onClick={() => removeItem(item._id)}><Trash2 className="w-4 h-4 mr-2" /> Remove</Button>
        </div>
      </CardContent>
    </Card>
  );
